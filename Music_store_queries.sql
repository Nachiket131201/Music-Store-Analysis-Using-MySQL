...
/*4: Which city has the best customers?Return a query that returns one city that has the highest sum of invoice totals.*/
  SELECT billing_city,SUM(total) AS InvoiceTotal FROM invoice
  GROUP BY billing_city ORDER BY InvoiceTotal DESC LIMIT 1;

/*5: Who is the best customer? Return a query that returns the person who has spent the most money.*/
  SELECT customer.customer_id, first_name, last_name, SUM(total) AS total_spending FROM customer
  JOIN invoice ON customer.customer_id = invoice.customer_id
  GROUP BY customer.customer_id ORDER BY total_spending DESC LIMIT 1;

/*6.Write query to return the email, first name, last name, & Genre of all Rock Music listeners.Return the list ordered alphabetically by email starting with A.*/
 Method 1:
...
/*10.find out the most popular music Genre for each country.Determine the most popular genre as the genre 
